...
#include "HCC/SP_QSP_HCC/ode/Param.h"
#include "HCC_core.h"
...
int main(int argc, char* argv[])
{
	...
	std::string _inputAntigenCore;
	std::string _antigenFileCore;
    std::string _inputAntigenMargin;
	std::string _antigenFileMargin;
	std::string _outPath;
	...
	try {
		...
		desc.add_options()
			...
			("antigen-file-core,ac", po::value<std::string>(&_inputAntigenCore), "core antigen file name")
            ("antigen-file-margin,am", po::value<std::string>(&_inputAntigenMargin), "margin antigen file name")
		...
		if (!vm.count("antigen-file-core"))
		{
			std::cout << desc << "\n";
			std::cerr << "no core antigen file specified!\n";
		}
		else {
			_antigenFileCore = _inputAntigenCore;
		}

		if (!vm.count("antigen-file-margin"))
		{
			std::cout << desc << "\n";
			std::cerr << "no margin antigen file specified!\n";
		}
		else {
			_antigenFileMargin = _inputAntigenMargin;
		}
		/*
	...
	//std::cout << "processed parameters" << std::endl;

	//create both tumor cores using two antigen dataset.
	HCC_Core simulation(_antigenFileCore, _antigenFileMargin);
