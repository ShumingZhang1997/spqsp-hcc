...
int main(int argc, char* argv[])
{
	...
	bool _antigenInclusion = false;
	std::string _inputAntigen;
	std::string _antigenFile;
	...
	try {
		...
		desc.add_options()
			...
			("seed,s", po::value<unsigned long>(&_seedVal)->default_value(0), "seed value (0 to use current timestamp)")
			...
			("antigen,A", po::bool_switch(&_antigenInclusion), "whether include antigen file")
			("antigen-file", po::value<std::string>(&_inputAntigen), "antigen file name")
		...
		if (_antigenInclusion) {
			if (!vm.count("antigen-file"))
			{
				std::cout << desc << "\n";
